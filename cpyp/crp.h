...
public:
	crp() :
			num_tables_(), num_customers_(), discount_(0.1), strength_(1.0), discount_prior_strength_(std::numeric_limits<double>::quiet_NaN()),
					discount_prior_beta_(std::numeric_limits<double>::quiet_NaN()), strength_prior_shape_(std::numeric_limits<double>::quiet_NaN()),
					strength_prior_rate_(std::numeric_limits<double>::quiet_NaN()) {
		check_hyperparameters();
	}

	crp(double disc, double strength) :
			num_tables_(), num_customers_(), discount_(disc), strength_(strength), discount_prior_strength_(std::numeric_limits<double>::quiet_NaN()),
					discount_prior_beta_(std::numeric_limits<double>::quiet_NaN()), strength_prior_shape_(std::numeric_limits<double>::quiet_NaN()),
					strength_prior_rate_(std::numeric_limits<double>::quiet_NaN()) {
		check_hyperparameters();
	}
	...
	crp(double d_strength, double d_beta, double c_shape, double c_rate, double d = 0.8, double c = 1.0) :
			num_tables_(), num_customers_(), discount_(d), strength_(c), discount_prior_strength_(d_strength), discount_prior_beta_(d_beta), strength_prior_shape_(c_shape),
...
//		strength_ = 0.34;
		...
		if (num_tables_ > 0)
			llh_ = log_likelihood(discount_, strength_);
	}
	...
	int increment_no_base(const Dish& dish, Engine& eng, double* logq) {
		...
		if (share_table) {
			...
			*logq += log((selected_table_prevcount - discount_) / (loc.num_customers() - 1 - loc.num_tables() * discount_));
		...
		if (loc.num_customers() == 1) {
			...
			if (logq) {
				...
				if (selected_table_postcount)
					*logq += log(p_share * (selected_table_postcount - discount_) / (loc.num_customers() - loc.num_tables() * discount_));
				else
					*logq += log(p_empty);
			}
			return delta;
...
//    return log_likelihood(discount_, strength_);
	...
	void update_llh_add_customer_to_table_seating(unsigned n) {
		...
		if (t == 1)
			llh_ += log(discount_) + log(strength_ / discount_ + num_tables_);
		if (n > 0)
			llh_ += log(n - discount_);
	}
	...
	void update_llh_remove_customer_from_table_seating(unsigned n) {
		...
		if (t == 1)
			llh_ -= log(discount_) + log(strength_ / discount_ + num_tables_ - 1);
		if (n > 1)
			llh_ -= log(n - discount_ - 1);
	}
	...
	double log_likelihood(const double& discount, const double& strength) const {
		...
		if (num_customers_) {  // if restaurant is not empty
			if (discount > 0.0) {  // two parameter case: discount > 0
				...
				lp += -lgamma(strength + num_customers_) + num_tables_ * log(discount) + lgamma(strength / discount + num_tables_);
				// above line implies
	...
	void resample_hyperparameters(Engine& eng, const unsigned nloop = 5, const unsigned niterations = 10) {
		...
		for (unsigned iter = 0; iter < nloop; ++iter) {
			if (has_strength_prior()) {
				s = slice_sampler1d([this,d](double prop_s) {return this->log_likelihood(d, prop_s);}, s, eng, -d + std::numeric_limits<double>::min(),
			...
			if (has_discount_prior()) {
				...
				if (s < 0.0)
					min_discount -= s;
				d = slice_sampler1d([this,s](double prop_d) {return this->log_likelihood(prop_d, s);}, d, eng, min_discount, 1.0, 0.0, niterations, 100 * niterations);
			}
		}
		s = slice_sampler1d([this,d](double prop_s) {return this->log_likelihood(d, prop_s);}, s, eng, -d + std::numeric_limits<double>::min(),
				std::numeric_limits<double>::infinity(), 0.0, niterations, 100 * niterations);
