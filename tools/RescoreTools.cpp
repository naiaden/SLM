...
//	SLM::AcousticSorter sorter;
    ...
    for (auto & r : referenceIds)
    {
    	...
    	// if get best hypothesis per nbest file
		{
    		...
    		for(auto & nbl : r.second.getNBestLists())
    		{
    			...
    			std::vector<std::string> t = sorter.sort(*nbl).getTokens();
    			fw.addLine(join(tpp.removeFillers(t, true), " "));
    		}
		}
    }
    ...
    std::cout << levenshtein_distance(ref, hyp) << " " << 1.0 - levenshtein_distance(ref, hyp)/(double) ref.size() << std::endl;
